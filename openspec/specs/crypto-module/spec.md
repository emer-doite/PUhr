...
The system MUST derive a 256-bit key from a PIN and 128-bit salt using Argon2id (m=65536KB, t=3, p=4). A KeyDeriver interface MUST abstract the implementation — JVM tests use a fake; instrumented tests use the real Argon2kt JNI.

#### Scenario: Same PIN + salt produces same key
- GIVEN a PIN and a 128-bit salt
- WHEN derive runs twice with identical PIN and salt
- THEN both outputs MUST be identical 256-bit keys

#### Scenario: Different salts produce different keys
- GIVEN the same PIN
- WHEN derive runs with two different salts
- THEN the resulting keys MUST differ
